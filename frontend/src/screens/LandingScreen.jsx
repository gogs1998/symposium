...
import { FigureCard, CitationCard, DisclosureBanner, MessageBubble } from '../components/ds'

const Label = ({ children, style }) => (
  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', letterSpacing: 'var(--tracking-caps)', textTransform: 'uppercase', color: 'var(--text-muted)', ...style }}>{children}</div>
)

/**
 * LandingScreen — the marketing front door. Vendored from the v2 design export
 * (ui_kits/landing/Landing.jsx) with demo globals replaced by real DS components
 * and the showcase strip fed by live roster data. "Enter the reading room" flips
 * the localStorage flag and hands off to the roster. Skippable.
 */
export function LandingScreen({ figures = [], onEnter }) {
  // Prefer a mixed showcase: a few historical figures then a creator, published only.
  const published = figures.filter((f) => f.status === 'published')
  const historical = published.filter((f) => f.category === 'historical')
  const creators = published.filter((f) => f.category === 'creator')
  const showcase = [...historical.slice(0, 3), ...creators.slice(0, 1)].slice(0, 4)
      ...
      <section id="promise" style={{ maxWidth: 'var(--content-max)', margin: '0 auto', padding: 'var(--space-8) var(--space-6)', display: 'grid', gridTemplateColumns: '5fr 6fr', gap: 'var(--space-7)', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <MessageBubble role="assistant" author="Charles Darwin"
            citations={[{ variant: 'book', source: 'On the Origin of Species', detail: 'ch. 4', excerpt: 'Natural selection acts solely by accumulating slight successive favourable variations.' }]}>
      ...
      <section id="roster" style={{ background: 'var(--surface-raised)', borderTop: '1px solid var(--stone-line)', borderBottom: '1px solid var(--stone-line)' }}>
        <div style={{ maxWidth: 'var(--content-max)', margin: '0 auto', padding: 'var(--space-8) var(--space-6)' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 'var(--text-2xl)', margin: '10px 0 0' }}>
              {total ? `${total} ${total === 1 ? 'voice' : 'voices'}, one rule.` : 'One rule, kept.'}
            </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 'var(--space-4)', marginTop: 'var(--space-6)', alignItems: 'start' }}>
            {showcase.map(({ chunkCount, fields, openers, imageUrl, format, ...card }) => (
              <FigureCard key={card.id} {...card} src={imageUrl} onClick={onEnter} />
            ))}
          </div>
        </div>
      </section>
